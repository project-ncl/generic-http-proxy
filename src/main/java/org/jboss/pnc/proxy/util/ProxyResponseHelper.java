...
import static org.jboss.pnc.proxy.util.HttpProxyConstants.FORBIDDEN_HEADERS;
...
import org.jboss.pnc.proxy.handler.ProxyCreationResult;
import org.jboss.pnc.proxy.handler.ProxyRepositoryCreator;
...
public class ProxyResponseHelper {
    ...
    public ProxyResponseHelper(
            HttpRequest httpRequest,
            ProxyConfiguration config,
            ProxyRepositoryCreator repoCreator,
            RepositoryService repositoryService,
            ContentRetrievalService contentRetrievalService,
            IndyObjectMapper indyObjectMapper,
    ...
    private ArtifactStore doGetArtifactStore(String trackingId, final URL url)
        ...
        try {
            ...
            if (trackingId != null) {
                ...
                try {
                    response = repositoryService
                            .getStore(PackageTypeConstants.PKG_TYPE_GENERIC_HTTP, "group", groupName);
                ...
                try {
                    response = repositoryService
                            .getRemoteByUrl(PackageTypeConstants.PKG_TYPE_GENERIC_HTTP, "remote", baseUrl);
    ...
    private ProxyCreationResult createRepo(String trackingId, URL url, String name)
        ...
        ProxyCreationResult result = repoCreator.create(
                trackingId,
                name,
                baseUrl,
                info,
        ...
        if (remote != null) {
            ...
            try (Response resp = repositoryService
                    .getStore(PackageTypeConstants.PKG_TYPE_GENERIC_HTTP, "remote", remote.getName())) {
                ...
                if (e.getResponse().getStatus() == HttpStatus.SC_NOT_FOUND) {
                    ...
                    try (Response resp = repositoryService.createStore(
                            PackageTypeConstants.PKG_TYPE_GENERIC_HTTP,
                            "remote",
                            indyObjectMapper.writeValueAsString(remote))) {
        ...
        if (hosted != null) {
            try (Response resp = repositoryService
                    .getStore(PackageTypeConstants.PKG_TYPE_GENERIC_HTTP, "hosted", hosted.getName())) {
                ...
                if (e.getResponse().getStatus() == HttpStatus.SC_NOT_FOUND) {
                    ...
                    try (Response resp = repositoryService.createStore(
                            PackageTypeConstants.PKG_TYPE_GENERIC_HTTP,
                            "hosted",
                            indyObjectMapper.writeValueAsString(hosted))) {
        ...
        if (group != null) {
            ...
            try (Response resp = repositoryService.createStore(
                    PackageTypeConstants.PKG_TYPE_GENERIC_HTTP,
                    "group",
                    indyObjectMapper.writeValueAsString(group))) {
    ...
    private void handleException(Exception e) throws IndyProxyException {
        if (e instanceof WebApplicationException) {
            logger.error(
                    "Create store error in repository service, status: {}, error: {}",
    ...
    private void doTransfer(
        ...
        Boolean isTracking = proxyUserPass != null && proxyUserPass.getUser() != null
                && proxyUserPass.getUser().endsWith(TRACKED_USER_SUFFIX);
        ...
        try {
            ...
            Uni<okhttp3.Response> responseUni = contentRetrievalService.doGet(
                    trackingId,
                    store.getType().name(),
                    store.getName(),
                    encodedPath);
