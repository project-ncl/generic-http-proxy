 ...
 */
package org.jboss.pnc.proxy.keycloak;
...
import org.jboss.pnc.proxy.config.KeycloakConfig;
...
public class KeycloakProxyAuthenticator {
    ...
    public boolean authenticate(UserPass userPass, HttpWrapper http)
        ...
        if (!keycloakConfig.isEnabled() || !httproxConfig.isSecured()) {
            logger.debug(
                    "Keycloak httprox translation authenticator is disabled OR httprox is running in unsecured mode. Skipping authentication.");
            return true;
        }
        ...
        if (result == null) {
            logger.info(
                    "No keycloak bearer token provided! This must either be in the password of a BASIC "
    ...
    protected AuthResult authenticateToken(HttpWrapper exchange, String tokenString)
        ...
        try {
            ...
            token = RSATokenVerifier
                    .verifyToken(tokenString, getHardcodedRealmKey(deployment), deployment.getRealmInfoUrl());
